...
#ifdef MMCBLKWRITE
mmc_images mmc_image_list[] = 
{
	{
		.name = "kernel",
		.mmcblk = MMCBLK0,
		.start_add = 1120*BLK_SIZE,
		.size = 8192*BLK_SIZE,   ///4m
	},
	{
		.name = "ramdisk",
		.mmcblk = MMCBLK0,
		.start_add = 9312*BLK_SIZE,
		.size = 4096*BLK_SIZE,// //2m
	},
	/*may be different with blk0*/
	{
		.name = "SDCARD",
		.mmcblk = MMCBLK0P1,
		.start_add = 0,
		//.size = 0xB509AA00, // 105798KB
	},
	{
		.name = "system",
		.mmcblk = MMCBLK0P2,
		.start_add = 0,
		.size = 0x12FE5800, // 300m
	},
	{
		.name = "DATA:",
		.mmcblk = MMCBLK0P3,
		.start_add = 0,
		.size = 0x40648800, // 1G KB
	},
	{
		.name = "CACHE:",
		.mmcblk = MMCBLK0P4,
...
#if 0
	{
		...
		.mmcblk = MMCBLK0P2,
		.start_add = 0xB11E00,
		.size = 0x6751800, // 105798KB
	},
	{
		.name = "userdata",
		.mmcblk = MMCBLK0P3,
		.start_add = 0x7263600,
		.size = 0x15E8B600, //358957KB
	},
	{
		.name = "cache",
		.mmcblk = MMCBLK0P4,
		.start_add = 0x1D0EEC00,
		.size = 0x19234200, // 411856KB
	}
#endif
};
#endif
...
#ifdef MMCBLKWRITE
const mmc_images *get_mmc_blk_byname(const char *name)
{
	if(mmc_image_list != NULL)
	{
		unsigned int i;
		for(i = 0; i< sizeof(mmc_image_list); i++)
		{
			mmc_images *p = &mmc_image_list[i];
			if(p->name != NULL)
			{
...
int open_mmcblk(const char *mmcblk)
{
	int fd = open(mmcblk, O_RDWR);
    if(fd < 0) 
	{
		fprintf(stderr,"error opening %s", mmcblk);
		return -1;
	}
	else
	{
...
#ifdef MMCBLKWRITE
/*added for mmcblk node mount*/
int mmcblk_mount_node(const mmc_images *mmcblk_node, const char *mount_point,
        const char *filesystem, int read_only)
{
	...
	sprintf(devname, "%s", mmcblk_node->mmcblk);
    if (!read_only) {
