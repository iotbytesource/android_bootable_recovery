...
Value *WriteRawImageFn(const char* name, State* state, int argc, Expr* argv[])
{
	...
	char *partition;
	char *filename;
	const mmc_images *mmclbk_node;
	int mmc_fd;
	/*partition name, KERNEL/RFS/SYSTEM...*/
	...
	/*find mmcblk node*/
	mmclbk_node = get_mmc_blk_byname(partition);
	if(mmclbk_node == NULL)
	{
		fprintf(stderr, "%s: no MMCBLK named \"%s\"\n", name, partition);
        result = strdup("");
        goto done;
	}
	/*open blk node and image file*/
	if(mmclbk_node->mmcblk != NULL)
	{
		mmc_fd = open_mmcblk(mmclbk_node->mmcblk);
		if(mmc_fd <0)
		{
			fprintf(stderr, "%s: can't open mmcblk \"%s\"\n",
                name, mmclbk_node->mmcblk);
	        result = strdup("");
	        goto done;
		}
	}
	...
	{
		fprintf(stderr, "%s: mmcblk is empty!\n",
            name);
        result = strdup("");
        goto done;
	}
	...
	/*find out the offset add and size*/
	unsigned long offset = mmclbk_node->start_add;
	unsigned long size = mmclbk_node->size;

	if(lseek(mmc_fd, offset, SEEK_SET) != offset)
	{
		fprintf(stderr, "can't get the start add:%d from mmcblk:%s", offset,mmclbk_node->mmcblk);
		  result = strdup("");
        goto done;
	}
    ...
    while (success && (read = fread(buffer, 1, BUFSIZ, f)) > 0) {
        wrote = write(mmc_fd, buffer, read);
		wrote_size += wrote;
		if((unsigned long)wrote_size >= size) 
			fprintf(stderr,"image read size:%d is over mmcblk size", wrote_size);
